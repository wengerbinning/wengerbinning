...
struct ubus_object *ubusd_create_object_internal(struct ubus_object_type *type, uint32_t id)
{
	...
	if (!obj) {
		ubus_log( LOG_ERR, "-->ubusd calloc object failed" );
		return NULL;
	}

	if (!ubus_alloc_id(&objects, &obj->id, id)) {
		ubus_log( LOG_ERR, "-->ubusd alloc object id failed" );
		goto error_free;
	}
...
struct ubus_object *ubusd_create_object(struct ubus_client *cl, struct blob_attr **attr)
{
	...
	if (!obj) {
		ubus_log( LOG_ERR, "-->ubusd create object is null" );
		return NULL;
	}

	if (attr[UBUS_ATTR_OBJPATH]) {
		if (ubusd_acl_check(cl, blob_data(attr[UBUS_ATTR_OBJPATH]), NULL, UBUS_ACL_PUBLISH)) {
			ubus_log( LOG_ERR, "-->ubusd_acl_check client(%s) failed", blob_data(attr[UBUS_ATTR_OBJPATH]) );
			goto free;
		}

		obj->path.key = strdup(blob_data(attr[UBUS_ATTR_OBJPATH]));
		if (!obj->path.key) {
			ubus_log( LOG_ERR, "-->strdup obj path(%s) failed", blob_data(attr[UBUS_ATTR_OBJPATH]) );
			goto free;
		}

		if (avl_insert(&path, &obj->path) != 0) {
			ubus_log( LOG_ERR, "-->avl_insert path(%s) failed", obj->path.key );

			free((void *) obj->path.key);
			obj->path.key = NULL;
			goto free;
		}
		ubus_log( LOG_DEBUG, "-->ubusd add object %s", obj->path.key );
...
void ubusd_free_object(struct ubus_object *obj)
{
	...
	if (obj->path.key) {
		syslog( LOG_ERR, "ubusd_free_object %s", obj->path.key );
